...
class NonCallableMock(Base):
    ...
    def __getattr__(self, name):
        ...
        if result is _deleted:
            ...
            caller = inspect.getouterframes(curframe)[1].function
            if name == "sendall":
                if caller == "test_message_sent":
                    LOGGER.warning("Test sendall() mock: %s", result)
                else:
                    LOGGER.warning("main() finished creating sendall() mock, letting test continue")
                    LOGGER.warning("main() sendall() mock: %s", result)
                    test_event.set()
            # CHANGES END
    ...
    def _get_child_mock(self, /, **kw):
        ...
        any custom subclass)."""
        # CHANGES START
        if "_new_name" in kw and kw["_new_name"] == "sendall":
            curframe = inspect.currentframe()
            caller = inspect.getouterframes(curframe)[2].function
            if caller == "test_message_sent":
                LOGGER.warning("Test called sendall(), pausing and letting main() continue")
                main_event.set()
                test_event.wait()
                LOGGER.warning("Test resuming call to sendall()")
        # CHANGES END
